...
subtitle: "Logistic regression and Mixture model"
...
+ implement SGLD algorithm in R with possibility to extend to other programming languadges  

+ recap logistic regression, Normal mixture model  

In the computer room, students will practice on the logistic regression example. Students are suggested to practice on the Mixture model example where the straightforward implementation of the SGLD is meant to fail.  
...
Consider the Logistic regression problem with input $x\in\mathbb{R}$ and output/labels $y\in\{0,1\}$. 

Consider there is available a dataset  $$\mathcal{S}_{n}=\left\{ z_{i}=\left(y_{i},x_{i}\right)\right\} _{i=1}^{n}$$ , with $y_{i}\in\{0,1\}$ and $x_{i}\in\mathbb{R}$.  
...
Let the dataset $\mathcal{S}_{n}$ has size $n=10^{6}$.  Assume that the real values for the unknown parameters $w$ is $w_{\text{true}}=(0.0,1.0)^\top$.  The dataset containing the examples to train the model are generated below, and stores in the array $z_{\text{obs}}$.  
...
Write an R function 'log_sampling_pdf(z, w)' for the log PDF of the sampling distribution
...
Code in R a function 'log_prior_pdf(w, mu= rep(0, length(w)), Sig2 = 1000*diag(length(w)) )' for the log PDF of the prior distribution of $w$ with mean default values $0.0$ and variance matrix default values $V=\text{diag}(1000,...,1000)$. You may use the R function ' dmvnorm\{mvtnorm\} '
...
Code in R, the learning rate function  learning_rate <- function(t, T_0 = 100, T_1 = 500, C_0 = 0.0001, s_0 = 0.5 )  
...
Compute the gradient of the log pdf of the sampling distribution with respect to $w$ at point $w=(-0.1,1.5)^\top$ (at the 1st example; i.e. $z_{1}$).  
...
gr <- grad_log_sampling_pdf(w = c(-0.1,1.5),z = z_obs[1,])
gr
```
...
The learning rate can be constant around $C_{0}=10^{-6}$ for the $1/3$ of the run, then it can delay $\varsigma=0.51$, and constant $C_{0}=10^{-6}$, while in the final $1/3$ it can be constant to a small value. (use the aforesaid provided function for the learning rate).  

The termination criterion is when the total number of iterations excesses $T_{\text{max}}=3\times 10^{3}$. 
...
Plot the trace plots of chains $\{w_1^{(t)}\}$ and $\{w_2^{(t)}\}$ against the iteration.  
...
The learning rate can be constant around $C_{0}=10^{-2}$ for the $1/3$ of the run, then it can delay $\varsigma=0.51$, and constant $C_{0}=10^{-2}$, while in the final $1/3$ it can be constant to a small value. (use the aforesaid provided function for the learning rate). 
...
\[
\widehat{w_{1}+w_{2}}=\sum_{t=1}^{T}\frac{\eta_{t}}{\sum_{t=1}^{T}\eta_{t'}}\left(w_{1}^{(t)}+w_{2}^{(t)}\right)
\]
...
weight_eta <- c()
for (i in 1:T) {
  weight_eta <- c(weight_eta, learning_rate(T_bunrin+t), T_0 = eta_T0, eta_T1 = 500, C_0 = eta_C, s_0 = eta_s)
}
weight_eta <- weight_eta/sum(weight_eta)
w_est = colMeans(w_chain_output,weight_eta)
w_est
```
...
# Mixture model (Homework challenge)  

```{r}
rm( list = ls() )
```

The following example is given as a homework practice. The problem is non convex, there are two modes (maxima) (true values) in the posterior distribution. You will see that you will not be able to visit both of the models (maxima) with one run.  You may need to perform several runs starting your algorithm from different seeds $w^{(0)}$.  
...
Let the dataset $\mathcal{S}_{n}$ has size $n=10^({}$.  Assume that the real values for the parameter $w$ is $w_{\text{true}}\in\{5,15\}$.  The dataset containing the examples to train the model are generated below, and stores in the array $z_{\text{obs}}$.  
```{r}
n_obs <- 10^(6)
w_true <- 5 
set.seed(2023)
...
Code a function `log_sampling_pdf(z, w, p1 = 0.5, phi=20, sig2 = 25)' that returns as a value the PDF of sampling distribution in log scale for a single example $z$.  

You may use the function  dnorm{stats} .

```{r}
log_sampling_pdf <- function(z, w, p1 = 0.5, phi=20, sig2 = 25) {
