...
lines(w_chain[,1], type="l", ylim = c(-2.1,1.2), ylim = c(-1.5,2.0), ylab ='w_{1}^{(t)}', xlab ='t', col=2, 
      ...
      cex.main=fntsz, 
      cex.sub=fntsz )
legend('topright', title="Algorithm", legend=c('online SGD', 'SVGD(k=10)'),  lty=1, col=c(1,2), 
       cex=fntsz)
dev.off()
...
lines(w_chain[,2], type="l", ylim = c(-2.1,1.2), ylab ='w_{2}^{(t)}', xlab ='t', col=2, 
      ...
      cex.main=fntsz, 
      cex.sub=fntsz )
legend('bottomright', title="Algorithm", legend=c('online SGD', 'SVGD(k=10)'),  lty=1, col=c(1,2), 
       cex=fntsz)
dev.off()
